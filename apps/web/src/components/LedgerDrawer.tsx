...
export function LedgerDrawer({ episodeId }: { episodeId: string }) {
  ...
  return (
    <Overlay title="Ledger — hash-chained audit trail" onClose={() => close(null)} width="640px">
      {q.isError && <div className="px-6 md:px-6 py-6 text-sm text-error">409 chain break detected — system should halt itself.</div>}
      {q.isLoading && <Skeleton />}
      {q.data && (
        <>
          <div className={`mx-6 md:mx-6 mt-6 md:mt-16 rounded-xl border px-4 md:px-4 py-3 md:py-2 text-xs ${q.data.valid ? "border-secondary text-secondary bg-secondary-container/20" : "border-error text-error bg-error-container"}`}>
            chain verification: {q.data.valid ? "valid ✓" : "BROKEN ✗"}
          </div>
          <ol className="space-y-0 px-6 md:px-6 py-6 md:py-6">
            {q.data.events.map((e) => (
              <li key={e.seq} className="relative border-l border-outline-variant pb-6 md:pb-6 pl-4 md:pl-6 font-mono text-[11px]">
          ...
          <div className="px-6 md:px-6 pb-6 md:pb-6 flex items-center gap-4">
            <button
              className="rounded-full border border-primary px-4 md:px-4 py-2 md:py-3 text-xs text-primary hover:bg-surface-container"
