...
| **Proof** | IMPLEMENTED | generator+pipeline+B0/B1+A1–A4+DEGRADED proven ≤ n=120 smoke w/ bootstrap CIs; **official run EXECUTED (final artifact 2026-08-30)** under `eval-preregistered-v1` — artifacts committed at `eval/results/20260830T105923Z/` [SIMULATED] |
...
| Eval 3×3000+ablations <10min | ✅ EXECUTED 2026-08-30 | official run complete (seed 42 × 8 arms incl. A1–A4+DEGRADED); artifacts `eval/results/20260830T105923Z/` (`results.json` + `tables.md`); exact wall-clock not recorded in artifacts (protocol target <10 min for runs) |
