...
- **v1.3: suite re-run at HEAD — GREEN.** 127 collected / **127 passed, 0 failed** (exit 0;
  e2e + ai_live excluded) against Postgres 16 + Redis 7 containers. Includes the new gates:
  COMPLAIN recall ≥90% + confidence fallback (TASK-054), Idempotency replay + error-envelope
  API tests, and kill-switch drain measurement (below). The earlier "STALE vs HEAD" caveat is
  resolved for this commit; CI execution on GitHub still pending (TASK-001).
- Web: 	tsc -b --noEmit 0 errors; vite build success; vitest 5/5 (formatINR).
...
- ~~`RISK_HELD` absent from synthetic mixture~~ → **RESOLVED v1.3 (TASK-053):** mixture rebalanced
  to include RISK_HELD (2%); demo slice re-fit verified green; amendment tag owed before official run.
...
- Kill-switch drain ≤1s was UNMEASURED → **MEASURED v1.3:** dedicated harness
