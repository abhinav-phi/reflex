...
- **Kill switch:** `POST /api/control/mode {halted}` (or button) → Redis flag → workers drain, scheduled actions cancelled (`CANCELLED_HALT`), banner red. Resume ⇒ re-plan fresh; nothing auto-replays blind. **Drain ≤ 1 s is now MEASURED (v1.3): 25.5 ms for 500 scheduled actions through the real DB path** (`tests/load/test_kill_switch_drain.py`); the dispatcher additionally re-checks the halted flag before every send.
...
> **v1.1 HONESTY ANNOTATION (strengthened v1.3):** every ₹ figure in this script (₹2,41,000 failed ·
> ~₹57,800 naive · ₹1,01,200 reflex · "42% vs 24% vs 7%") is a **DESIGN TARGET for narration**,
> not a measured result. Until TASK-033's official run completes and its `[SIMULATED]` results are
> committed: (a) presenters must either show the live smoke-labeled counters or say "targets,
> evaluation pre-registered"; (b) any narration cell marked **[TARGETS]** below must be spoken as
> a target, never as an achieved metric; (c) if a judge asks for `eval/results/`, show the
> committed B1 tuning search + smoke artifacts and state plainly that the official run is pending.
> Never read these numbers as achieved metrics.
...
| 3:30–4:15 | `/results` — three-arm table w/ CIs + ablation bars (`[SIMULATED]` badges visible) **[TARGETS — say "pre-registered targets" unless official results are committed]** | "42% vs 24% vs 7%, cheaper per ₹100, fewer complaints. Ablations show which AI component buys which points. Pre-registered, one command, reproducible." |
