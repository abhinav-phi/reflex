...
> **SYNC (post official run + production hardening, 2026-08-30; infra sync 2026-09-01):** implementation status: **59/59 tracked tasks executed** (all partials closed). **The official pre-registered evaluation HAS EXECUTED** — protocol `eval-preregistered-v1`, seed 42, N=3000 episodes × 8 arms, artifacts committed at **`eval/results/20260830T105923Z/`** (`results.json` + `tables.md`), ALL VALUES [SIMULATED]. Headline actuals: Reflex **33.83%** CI[29.36,38.75] · cost ₹0.24/₹100 · complaints 0.2% vs B1 25.06% / ₹0.12 / 0.533% vs B0 4.11%; incremental vs B1 **+8.77 pp** CI[+4.49,+13.28] — **G1 ≥ +15 pp gate MISSED**, G2/G3 pass. Honest record incl. degraded==full and A2 anomaly: **docs/limitations.md**. Authoritative status: PRD §22 + ImplementationPlan snapshot + Tracker. **Production deployment live:** UI https://reflex-recover.vercel.app · API https://reflex-api-production.up.railway.app (Redis on Railway private network; **PostgreSQL on Aiven Free** since 2026-09-01 — migrated bit-exact from the filled 500 MB Railway volume; pools capped to Aiven's 20-connection budget; ledger chain re-stamped + per-episode trail verification fixed — ops runbook in **MIGRATION.md**).
...
| **Status** | Design/docs complete (10 files, 59 tracked tasks) · **build: 35 [x] done · 19 partial ([/] 18 + [~] 1) · 5 [ ] open — of 59 tracked tasks** (see §19) · **official pre-registered eval EXECUTED (final artifact 2026-08-30** — `eval/results/20260830T105923Z/`, all values [SIMULATED]) |
...
| Official eval run | ✅ **EXECUTED 2026-08-30** under `eval-preregistered-v1` — artifacts `eval/results/20260830T105923Z/` [SIMULATED]; Reflex 33.83% CI[29.36,38.75], B1 25.06%, B0 4.11%, incremental +8.77 pp CI[+4.49,+13.28] *(G1 ≥+15 pp missed; G2/G3 pass)*; loaded into the production Results page; superseded attempts archived with README |
...
**The numbers it produced** (official run 2026-08-30, pre-registered, reproducible, `[SIMULATED]`; artifacts `eval/results/20260830T105923Z/`): **33.83% recovery CI[29.36,38.75] vs 25.06% tuned-naive vs 4.11% organic · ₹0.24 vs ₹0.12 per ₹100 recovered · 0.2% vs 0.533% complaints · median TTR 10.1 h (B0 organic 39.2 h)** — with ablations A1–A4 + DEGRADED proving which component buys which points (timing ≈ +5.9 pp; EV-off anomaly at 39.93% disclosed — higher raw recovery at ~2× cost and ~1.8× complaints), and a published cohort of **1,189 episodes** where the system correctly refuses to act. Pre-registered targets were ~42%/~24%/~7%; actuals came in under them and the G1 ≥+15 pp incremental gate was missed (+8.77 pp) — reported honestly.
