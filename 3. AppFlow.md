...
**Version:** 1.1 · Terminology per `1. PRD.md` / `2. TechSpec.md`.
...
2. **Diagnose (t+0.1s):** rules engine matches fuzzy "insufficient balance" pattern → `INSUFFICIENT_FUNDS`, confidence 0.97, method RULE. (If rules missed: LLM tail per TechSpec §7 AI-1; confidence < 0.6 ⇒ UNKNOWN_AMBIGUOUS ⇒ conservative `WAIT 4h`.) Ledger #2.
3. **Plan (t+0.3s):** Brain enumerates candidates for INSUFFICIENT_FUNDS: `WAIT_TO_16:00_THEN_UPI_LINK` (salary-cycle + evening funds pattern → p=0.34, EV +₹98), `RETRY_NOW_UPI` (p=0.12, EV −₹180 after annoyance), `SMS_NOW` (p=0.18, EV +₹41), `STOP` (EV 0). All four persisted with full EV breakdown. Ledger #3.
4. **Guard (t+0.4s):** Shield sequence: kill-switch? off → mode? autonomous → episode cap 0/4 ✓ → customer contacts today 0/2 ✓ → quiet hours? 09:14 ok, but scheduled 16:00 ok ✓ → budget ₹0.80 < ₹5,000 ✓ → suppressed? no → value ₹299 < ₹50,000 ✓ ⇒ **PASS**. (Any fail ⇒ BLOCKED(reason) ledgered; approval triggers ⇒ queue.) Ledger #4.
...
5. Every action stamped `mode=DEGRADED`; recovery rate visibly dips (~9 pts) but continues.
...
- **Kill switch:** `POST /api/control/mode {halted}` (or button) → Redis flag → workers drain (≤ 1 s), scheduled actions cancelled (`CANCELLED_HALT`), banner red. Resume ⇒ re-plan fresh; nothing auto-replays blind.
...
> **v1.1 HONESTY ANNOTATION:** every ₹ figure in this script (₹2,41,000 failed · ~₹57,800 naive ·
> ₹1,01,200 reflex · "42% vs 24% vs 7%") is a **DESIGN TARGET for narration**, not a measured
> result. Until TASK-033's official run completes and its `[SIMULATED]` results are committed,
> presenters must either show the live smoke-labeled counters or say "targets, evaluation
> pre-registered." Never read these numbers as achieved metrics.
...
|---|---|---|
| 0:00–0:30 | Full-screen dashboard, red counter **"FAILED TODAY ₹2,41,000 · RECOVERED ₹0 · NAIVE WOULD GET ~₹57,800"**; failure cards streaming | "This is SipDaily. ₹2.4 lakh of subscription revenue failed today — UPI, cards, mandates. Merchants answer with silence or spam. Both leak money." |
...
| 1:00–2:30 | **Live:** toggle Reflex ON → chips appear (`INSUFFICIENT_FUNDS — salary-week`, `ISSUER_DOWNTIME`, `MANDATE_REVOKED`) → open EV drawer on ₹299 episode → a ₹48,000 episode lands in **approval queue**, approve it live → green counter climbs toward **₹1,01,200** past naive's **₹57,800** | "Every action shows its math and its guardrail state. High-value anything waits for a human. Watch recovery beat the naive baseline on the same batch." |
...
| 3:30–4:15 | `/results` — three-arm table w/ CIs + ablation bars (`[SIMULATED]` badges visible) | "42% vs 24% vs 7%, cheaper per ₹100, fewer complaints. Ablations show which AI component buys which points. Pre-registered, one command, reproducible." |
...
|---|---|---|---|
| §2 | Onboarding (keys→webhook→guardrails→mode) | UNVERIFIED LIVE | 5 API routes + UI page implemented; RP ₹1 connectivity check requires real 
zp_test_ keys |
...
| §5 steps 1–2 | Ingest → dedup → episode; rules dx | **IP VERIFIED** | webhook tests (sig-401, dup-200, zero-amount reject) + storm integration (1000→214, 0 dupes); LLM-tail path verified offline-degraded |
...
| §8 inj-2 | Webhook storm 1,000→214 | PARTIAL | dedup logic integration-tested at ingest level; live /inject/webhook_storm untested |
...
| §11 | Kill switch drain ≤1s | UNVERIFIED | halt flag checked per dispatch tick; scheduled→CANCELLED_HALT + episodes→HALTED coded; timing never measured |
