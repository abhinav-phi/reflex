...
**Version:** 1.1 · Postgres 16 · Three schemas enforce ADR-004 separation: `runtime` (agent world), `replay` (hidden simulator truth), `eval` (evidence). **Agent services connect with a DB role that cannot read `replay` tables containing hidden parameters.**
...
## 15. As-Built Deltas & Verification (v1.1)
...
- Exports with `[SIMULATED]` headers: NOT IMPLEMENTED.
