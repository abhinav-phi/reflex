...
**Version:** 1.1 · Companion to `1. PRD.md`. Terminology identical across all 8 docs.
...
| **Proof** | PARTIAL | generator+pipeline+B0/B1+A1–A4+DEGRADED proven ≤ n=120 smoke w/ bootstrap CIs; **official N=3000×3 run BLOCKED** by host Docker instability |

### 21.2 As-built stack deviations
1. **Packaging:** single installable distribution 
eflex via package-dir mappings
   (packages/core→
eflex.core, … pps/eval→
eflex.eval, data/generators→data.generators);
   console scripts 
eflex-worker / 
eflex-eval / 
eflex-seed.
...
4. **Redis client:** synchronous 
edis-py; 
edis.asyncio avoided entirely.
...
6. **Rate limiting:** fixed-window Redis counters checked per route bucket from a shared helper
   (decorator-style calls in handlers) — global middleware + Idempotency-Key response-store NOT finished (Rules §1.4 partial).
...
| Ingestion burst p95 <800ms / 0 dupes | ✅ tested | 5000 events → 3000 unique, p95 <800ms |
| Eval 3×3000+ablations <10min | ❌ NOT YET RUN | smoke only (n≤120); official run pending stable host |
...
- Suite: **120 collected**; last complete run **119 passed / 1 skipped** (i_live holdout needs
  LLM key) at commit 115a5c0. **STALE vs HEAD** — later changes (suppression lock/savepoints,
  CI arrays, async webhook body, pool sizing) not yet re-run.
- Web: 	sc -b --noEmit 0 errors; vite build success; vitest 5/5 (formatINR).
- Playwright spec authored (	ests/e2e/wow.spec.ts); browsers not installed → 0 executed.
...
- Ledger appends support a fast single-writer mode (ast_ledger) used by Proof; hash semantics
  identical; runtime keeps the advisory-serialized path.
- Docker Desktop on the build host crashed twice under eval load (environment, not product code).
