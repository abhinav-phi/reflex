...
| **Status** | Design/docs phase complete (8 files, 52 tasks defined) · build not started |
...
|---|---|---|---|---|---|
| AI-1 | Diagnosis NLU | Canonical root-cause classification of messy decline strings | Same cause surfaces as different issuer strings; lookup can't cover the ~25–30% tail | ≥85% accuracy on 500-case held-out set | conf < 0.6 ⇒ UNKNOWN_AMBIGUOUS ⇒ conservative default; invalid JSON ⇒ 1 retry ⇒ fallback |
...
| Backend | Python 3.12 + FastAPI 0.111 | Async, Pydantic boundaries |
...
| **Synthetic universe** | 3,000 customers: LTV bands, salary days (1–7 cluster), 70% Hinglish, 3% DND; failure-code mixture (INSUFFICIENT_FUNDS 34% … ambiguous tail 6%) with 5–8 issuer-string paraphrases per code |
...
| Audit & control | FR-010, 012–014, 018: hash-chained ledger + audit drawer; approval queue; kill switch & modes; degraded mode; suppression/complaint handling | Chain tamper detected; kill ≤1s drain; zero post-complaint contact |
| Outcomes & learning | FR-015: watch windows, attribution, policy credit | 100% episodes reach terminal state |
| Evaluation & demo | FR-016–017, 019–020: B0/B1 baselines, pre-registered eval harness + ablations, metrics views, demo/failure-injection controls | One-command repro; demo ≤10 min end-to-end |
...
|---|---|---|
| A1 | Rules-only diagnosis (LLM off) | Value of AI-1 on the ambiguous tail |
...
Plus: **losing cohort** — low-value (<₹150) ephemeral failures where Reflex correctly declines to act (contact cost > EV) while naive wastes spend; AI suite gates (dx ≥85%, validator 100%, COMPLAIN precision ≥95%, injection corpus 100% safe — CI-blocking); load test (5k-event burst, 40% dupes, zero dup episodes, p95 <800ms).
...
**Should have:** full ablation suite (min A2), voice-call sim, policy v2 learning, learning curve. **Nice:** mandate re-registration journey, B2B invoice flavor, policy profiles.
...
**4 working days · 3 engineers** (A: Data/Sim/AI+Eval · B: Backend/Agent+Security · C: Frontend/Design) · 52 tasks · 9 phases:
...
| Item | Status |
|---|---|
| 8 design docs | ✅ Finalized, cross-checked |
| Build | ❌ Not started — all 52 tracker tasks `[ ]` |
| Eval protocol tag | ⏳ Must exist **before first results run** (TASK-005) |
| Razorpay subscriptions "charge now" endpoint | `[TBD — verify existence/naming]`; fallback = new order + link |
| LLM model choice | `[Decision Required]` (MVP assumption: GPT-4o-mini-class) |
...
| Milestones ahead | `eval-preregistered-v1` → `v1.0-submission` |
...
**Current state:** design complete (8 docs · 52 tasks · 4-day plan · 3 engineers); build begins at TASK-001; eval protocol must be tagged before the first number exists.
...
| AI gates | dx ≥85% · validator 100% · COMPLAIN ≥95% · injection corpus 100% safe |
